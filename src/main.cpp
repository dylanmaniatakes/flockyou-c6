...
// Target hardware: Seeed Studio XIAO ESP32-C6 with a piezo buzzer on D2.
// No display, NeoPixel, or hardware GPS is required. A connected phone may
// optionally provide location through the dashboard's browser Geolocation API.
// ============================================================================
...
// Use the XIAO header label instead of a raw GPIO number. D2 was GPIO3 on the
// original XIAO ESP32-S3, but it is GPIO2 on the XIAO ESP32-C6. Referring to
// D2 preserves the physical wire position across the board change.
#define BUZZER_PIN D2

// BLE scanning
#define BLE_SCAN_DURATION_MS 2000  // NimBLE 2.x expresses scan duration in ms
#define BLE_SCAN_INTERVAL_MS 3000  // start-to-start interval between scans
...
// Atomic publish of a location supplied by the connected phone's browser.
// This is software-only; the ESP32-C6 has no GPS module connected.
...
// NimBLE-Arduino 2.x renamed its scan callback base class and made scan
// results const. The detector only reads advertisements, so const is the
// correct ownership model and avoids accidental mutation of library data.
...
fetch('/api/stats').then(r=>r.json()).then(s=>{let g=document.getElementById('sG'),gl=document.getElementById('sGL');if(s.gps_src==='phone'){g.textContent=s.gps_tagged+'/'+s.total;g.style.color='#22c55e';gl.textContent='PHONE';}else{g.textContent='TAP';g.style.color='#ef4444';gl.textContent='PHONE GPS';}}).catch(()=>{});}
...
static void fySetupServer() {
    ...
    fyServer.on("/api/stats", HTTP_GET, [](AsyncWebServerRequest *r) {
        ...
        const char* gpsSrc = fyGPSIsFresh() ? "phone" : "none";
        char buf[256];
    ...
    // API: Receive optional location from the connected phone's browser.
...
void setup() {
    ...
    printf("  Buzzer: %s\n", fyBuzzerOn ? "ON" : "OFF");
    printf("  Location: optional phone/browser GPS only\n");
    printf("========================================\n");
    ...
    fyBLEScan->start(BLE_SCAN_DURATION_MS, false);
...
void loop() {
    ...
    // BLE scanning cycle
    if (millis() - fyLastBleScan >= BLE_SCAN_INTERVAL_MS && !fyBLEScan->isScanning()) {
        fyBLEScan->start(BLE_SCAN_DURATION_MS, false);
        fyLastBleScan = millis();
    }

    if (!fyBLEScan->isScanning() && millis() - fyLastBleScan > BLE_SCAN_DURATION_MS) {
